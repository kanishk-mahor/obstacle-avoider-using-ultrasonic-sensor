...
long n;
float dis,dis_cm,response,offset,height;
...
if(sampleidx>SAMPLE_SIZE)
	...
	samples[sampleidx]= dis;
offset = averageSamples(samples);
	}
  ...
  while(1)
  {
  	...
  	GPIOD->BSRR |= GPIO_Pin_7;
		...
		samples[sampleidx]= dis;
		dis_cm = averageSamples(samples);
		height = offset - dis_cm;
		if(dis_cm>=5.5&&dis_cm<=6.5)
		{
